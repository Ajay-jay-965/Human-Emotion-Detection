...
from keras.layers import Conv2D, MaxPooling2D, BatchNormalization  # , Averagepooling
...
test_y = test_y/np.std(test_y, axis=0)
...
model.fit(X_train, train_y, batch_size=batch_size,
          epochs=epochs, verbose=1, validation_data=(X_test, test_y), shuffle=True)

# saving model
fer_json = model.to_json()
with open("fer.json", "w") as join_file:
    join_file.write(fer_json)
